...
public class Book : MonoBehaviour
{
    ...
    void Awake()
    {
        var pageJsonStr = Resources.Load<TextAsset>("PageJson").text;
        print(pageJsonStr);
        var pageJson = JsonUtility.FromJson<PageJson>(pageJsonStr);
        bookPages = new Sprite[pageJson.count];
        Texture2D bgTexture2D = Resources.Load<Texture2D>(pageJson.bg);
        background = Sprite.Create(bgTexture2D, new Rect(0, 0, bgTexture2D.width, bgTexture2D.height), Vector2.zero);
        for (int i = 0; i < pageJson.count; i++)
        {
            print(pageJson.pages[i]);
            Texture2D texture2D = Resources.Load<Texture2D>(pageJson.pages[i]);
            bookPages[i] = Sprite.Create(texture2D, new Rect(0, 0, texture2D.width, texture2D.height), Vector2.zero);
        }
    }
